...
class TreatmentGoals:
    ...
    def __post_init__(self) -> None:
        total = self.w_motor + self.w_non_motor + self.w_duration
        if abs(total - 1.0) > 1e-6:
            raise ValueError(
                f"Treatment goal weights must sum to 1.0, got {total:.6f}"
            )
        if any(w < 0 for w in [self.w_motor, self.w_non_motor, self.w_duration]):
    ...
    def default(cls) -> TreatmentGoals:
        ...
        return cls(w_motor=0.33, w_non_motor=0.33, w_duration=0.34, non_motor_diary_ratio=0.5)
    
    @classmethod
    def motor_focused(cls) -> TreatmentGoals:
        """Prioritize motor symptoms."""
        return cls(w_motor=0.55, w_non_motor=0.35, w_duration=0.10, non_motor_diary_ratio=0.5)
    
    @classmethod
    def quality_of_life_focused(cls) -> TreatmentGoals:
        """Prioritize non-motor symptoms."""
        return cls(w_motor=0.20, w_non_motor=0.70, w_duration=0.10, non_motor_diary_ratio=0.65)
